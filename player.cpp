...
Player::Player(int money, int playerIdentity, string name) {
...
int Player::dealerTurn(){
    if((getHandTotals().at(0) >= 17) || (getHandTotals().at(getHandTotals().size() - 1) >= 18)) {
        return stand();
    }
    else {
        return getCard();
...
void Player::cardCount(Card * discard) {
    switch (playerIdentity) {
        ...
        case 3:
            ...
            if (discard->getNumericValue() == 2 || discard->getNumericValue() == 7) {
                runningCount += .5;
            }
                // If the top card of the other player is a Jack, Queen, King, or Ace, subtract one from the running count
            else if (discard->getNumericValue() == 3 || discard->getNumericValue() == 4 || discard->getNumericValue() == 6) {
                runningCount += 1;
            }
            ...
            else if (discard->getNumericValue() == 10 || discard->getNumericValue() == 1) {
                runningCount -= 1;
            }
            discardTally++;
            break;
